...
class PostDetailsViewController: UIViewController {
    ...
    private func setupViews() {
        ...
        self.postDescription.text = post?.body
...
extension PostDetailsViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.configureCell(album, indexPath.row)
        return cell
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        // Handle selection of a row (image) in the table view
        let coordinator = ImagePreviewCoordinator.init(navigationController: self.navigationController, photoModel: self.album?.photos?[indexPath.row])
        coordinator.start()
    }
}
